...
pot = filedialog.askdirectory(initialdir='/media/vid/SILICON 16G/')
    ...
    if a[-4:] == '.ASC':
        ...
        if '_' in a:
            ...
            try:
                ...
                print('razdelim')
                kot = float(b[0])
                key = b[1] #float(b[1][0]) (v serijah)
                print('čez')
                xdata, ydata = beri(pot + '/' + a)
                if kot > 100:
                    param = [-0.0919, 0.692, 0.2635, 124.95, 0.424, 0.9875, 0.8944]

                param, err = sp.curve_fit(f, xdata[spod:zgor], ydata[spod:zgor], param)
                print('fitam ' + a)
